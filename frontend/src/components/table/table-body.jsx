...
import { deleteTr } from '@/actions/transactions';
  ...
  return (
    <tbody id="tableBody" className="table-group-divider">
      {transactions.map((transaction, i) => (
        <tr
          key={transaction._id}
          className={transaction.status === 'pending' ? 'table-danger' : ''}
        >
          <td>{i + 1}</td>
          <td>{transaction.description}</td>
          <td className={transaction.type === 'Cr' ? 'text-success' : 'text-danger'}>
          ...
          <td className="text-center">
            ...
            <div className="btn-group">
              <Link to={`/edit-transaction/${transaction._id}`} className="btn btn-primary btn-sm">
                Edit
              </Link>
              <button
